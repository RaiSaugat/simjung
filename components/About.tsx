...
const About: React.FC = () => {
  ...
  return (
    <section 
      ...
      className="relative" 
      style={{ height: isMobile ? 'auto' : `${ABOUT_CHAPTERS.length * 100}vh` }}
    >
      <div className={`${isMobile ? 'relative py-24' : 'sticky top-0 h-screen'} w-full overflow-hidden bg-white`}>
        ...
        >
          <div 
            ...
            style={{ 
              transform: isMobile ? 'none' : `scale(${1 + scrollProgress * 0.2}) rotate(${scrollProgress * 5}deg)`
            }}
          >
            STORY
          </div>
        ...
        {!isMobile && (
          <div className="absolute left-12 bottom-12 z-50 flex flex-row items-center gap-6 bg-white/70 backdrop-blur-md px-6 py-3 rounded-full border border-sky-50 shadow-sm">
            <span className="text-[10px] font-black uppercase tracking-widest text-[#1E1B4B] whitespace-nowrap">Current Altitude</span>
            ...
            <span className="text-sm font-black text-[#1E1B4B] min-w-[50px]">{activeAltitude}m</span>
          </div>
        )}
        ...
        <div 
          ...
          style={{ 
            transform: isMobile ? 'none' : `translateX(-${scrollProgress * (ABOUT_CHAPTERS.length - 1) * 100}%)`,
            transition: isMobile ? 'none' : 'transform 0.1s linear'
          }}
        >
          {ABOUT_CHAPTERS.map((chapter, i) => (
            <div key={i} className={`min-w-full ${isMobile ? 'h-auto' : 'h-full'} flex flex-col lg:flex-row items-center justify-center px-6 md:px-12 lg:px-24 relative`}>
              ...
              <article className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-32 items-center w-full max-w-[1400px] mx-auto z-10">
                ...
                <div className="lg:col-span-5 order-2 lg:order-1">
                  <div className="flex items-center gap-4 mb-6 lg:mb-8">
                    <div className={`w-8 lg:w-12 h-[1px] ${chapter.color.replace('border-', 'bg-')}`}></div>
                    <span className={`font-black tracking-[0.4em] lg:tracking-[0.5em] uppercase text-[9px] lg:text-[10px] ${chapter.accent}`}>
                      {chapter.tag}
                    </span>
                  </div>
                  ...
                  <h2 className="text-4xl md:text-5xl lg:text-7xl font-black text-[#1E1B4B] tracking-tighter leading-[0.9] mb-8 lg:mb-10">
                    ...
                    <span className="font-serif italic font-normal text-gradient">{chapter.title}</span>
                  </h2>
                  ...
                  <div className="flex items-center gap-6 lg:gap-8">
                    <div className={`w-12 h-12 lg:w-14 lg:h-14 rounded-xl lg:rounded-2xl ${chapter.bg} flex items-center justify-center text-[#1E1B4B] border border-white shadow-sm`}>
                ...
                <div className="lg:col-span-7 order-1 lg:order-2 flex justify-center items-center">
                  <div className="relative aspect-[4/5] lg:aspect-[3/4] w-full max-w-[420px] lg:max-h-[65vh] rounded-[2rem] lg:rounded-[3rem] overflow-hidden shadow-2xl group border-4 lg:border-[8px] border-white bg-slate-50">
                    ...
                    <div className="absolute bottom-6 left-6 lg:bottom-10 lg:left-10 text-white">
                      <div className="w-4 lg:w-6 h-[1px] bg-white/40 mb-3"></div>
                      <span className="text-[8px] lg:text-[9px] font-black uppercase tracking-widest opacity-80 block mb-1">Peak Milestone</span>
                      <span className="text-lg lg:text-xl font-black tracking-tight text-sky-200">Verified Excellence</span>
                    </div>
                  </div>
                </div>
              </article>
                  ...
                  {ABOUT_CHAPTERS.map((_, dotIdx) => (
                    <div 
                      ...
                      className={`h-1 rounded-full transition-all duration-500 ${
                        Math.round(scrollProgress * (ABOUT_CHAPTERS.length - 1)) === dotIdx 
                          ? 'w-12 bg-sky-400' 
                          : 'w-3 bg-slate-100'
                      }`}
                    />
        ...
        {!isMobile && (
          ...
          >
            <span className="text-[10px] font-black uppercase tracking-widest text-slate-300">Begin Scroll</span>
